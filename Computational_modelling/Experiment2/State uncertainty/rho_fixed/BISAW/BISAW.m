...
% Vector conatining probabilities of the last b trials other than the most recent one 
% We initialise it outside the loop to speed up the program
prob_lastb = (ones(b,1)-pr)/(b-1); % this will serve as the probabilities of the last b trials other than the most recent one 
...
for n = 1:N
    ...
    ESV(1) = Compute_one_ESV(R0, b, m, w, pr, prob_lastb); % estimated subjective value of the promoted mapping
    ESV(2) = Compute_one_ESV(R1, b, m, w, pr, prob_lastb); % estimated subjective value of the promoted mapping
