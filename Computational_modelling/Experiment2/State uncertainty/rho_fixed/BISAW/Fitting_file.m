...
w0_vals = rand(numOfParamInit,1); % replace with 30!!!
...
for subj = 1:J 
    ...
    for i_b = 1:size(b_vals,1)
        ...
        parfor ii = 1:size(w0_vals,1)
            ...
            pr0 = pr0_vals(ii);            
            T0 = T0_vals(ii); 
            [w_fit,pr_fit,T_fit,LogML] = BISAW_fitting([w0,pr0,T0],rho,b,N,Ns,Dataj);
    ...
    fprintf(datafilepointer1,'%i %i %1.4f %1.4f %1.4f %1.2f %1.2f %1.2f\r\n', ...
            subj+2, ...
            b_bestfit, ...
            w_bestfit, ...
            pr_bestfit, ...   
