function [alpha_fit,T_fit,LogML] = SARSA_fitting(params_init,N,Ns,myfilename)
%[alpha_fit,T_fit,LogML] = SARSA_fitting([0.1,0.1],N,Ns,Dataj);
...
function NeglogML = MLE_RL(free_params,N,Ns,O,A,R)
    ...
    % sampled action from PWRL given previous actions up to n-1 to use to estimate the likelihoods of selected actions  
    ...
    % Equivalent to running this for loop:    
    %     logML = 0;
    %     for n = 1:N
