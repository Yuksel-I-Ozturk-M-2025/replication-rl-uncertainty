...
Acc_SARSA = zeros(N, J); % For each subject's fitted parameters, one set of learning curves was obtained by averaging over nb_rep (100) simulations of the model 
...
R_rel_SARSA = zeros(N, J); % For each subject's fitted parameters, one set of learning curves was obtained by averaging over nb_rep (100) simulations of the model 
...
for subj = 1:J
    ...
    % extract observations for the current subj (MODIFY PATH AS NEEDED)
    ...
    Acc_SARSA(:, subj) = mean(Acc_all, 2);
    R_rel_SARSA(:, subj) = mean(R_rel_all, 2);
...
save('./Results/Perf_SARSA_fit.mat','Acc_SARSA','R_rel_SARSA');
