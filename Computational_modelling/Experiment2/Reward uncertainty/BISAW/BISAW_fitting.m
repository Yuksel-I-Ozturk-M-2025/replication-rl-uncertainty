function [w_fit, pr_fit, T_fit, LogML] = BISAW_fitting(params_init,b,N,Ns,myfilename)
%[w_fit,pr_fit,T_fit,LogML] = BISAW_fitting([0.8,0.8,10],b,N,Ns);
...
O = MatData(:,4);
A = MatData(:,5);
A(101:200) = 1-A(101:200); % Because A (i.e. resp) encodes whether it the correct dark (A=0) or light response (A=1) was selected, and this encoding remains the same after the switch (so needed to adjust A) 
R = MatData(:,7);
...
function NeglogML = MLE_RL(free_params, b, N, Ns, O, A, R)
    ...
    for ii = 1:Ns
        ...
        for n = 1:N   
            ...
            R0 = R((Trials < n) & (A==0) & (O==O(n))); % reward history for action a = 0 and current state O(n)
            R1 = R((Trials < n) & (A==1) & (O==O(n))); % reward history for action a = 1 and current state O(n)
            ...
            elseif (N0 == 1) 
                ...
                I0 = N0 + 1 - sample_discrete(repmat(Prob(:, N0), [1, m])); 
            ...
            elseif (N1 == 1) 
                ...
                % draw a sample of m past trials following the 'Prob' distribution 
                I1 = N1 + 1 - sample_discrete(repmat(Prob(:, N1), [1, m]));
