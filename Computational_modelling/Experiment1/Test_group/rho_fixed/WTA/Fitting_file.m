...
alpha0_vals = rand(numOfParamInit,1); % replace 2 with 30
T0_vals = rand(numOfParamInit,1); % replace 2 with 30
...
for subj = 1:J
    ...
    parfor ii = 1:size(alpha0_vals,1)
        ...
        T0 = T0_vals(ii);
        [alpha_fit,T_fit,LogML] = WTA_fitting([alpha0,T0],rho,N,Ns,Dataj);
    ...
    % Record Matj into a text file (record outside parfor to avoid errors)
    for nn = 1:size(alpha0_vals,1)
        
        % Write results to file:   
        fprintf(datafilepointer2,'%1.3f %1.3f %1.4f %1.3f %1.3f\r\n', ...
                Matj(nn,1), ...
                Matj(nn,2), ...
                Matj(nn,5), ...
                Matj(nn,3), ...
