...
Acc_WTA = zeros(N, J); % For each subject's fitted parameters, one set of learning curves was obtained by averaging over nb_rep (100) simulations of the model 
...
R_rel_WTA = zeros(N, J); % For each subject's fitted parameters, one set of learning curves was obtained by averaging over nb_rep (100) simulations of the model 
...
alpha_vals = MatFittedParams(:,3);
T_vals = MatFittedParams(:,4);
...
for subj = 1:J
    ...
    for ii = 1:nb_rep
        ...
        [A, R, Acc, Qt] = WTA(O, N, rho, alpha, T, r0);
        % Overall accuracy
        Acc_avg(ii) = mean(Acc); 
        Acc_all(:, ii) = Acc;
    ...
    Acc_WTA(:, subj) = mean(Acc_all, 2);
    R_rel_WTA(:, subj) = mean(R_rel_all, 2);
...
save('.\Results\Perf_WTA_fit.mat','Acc_WTA','R_rel_WTA');
