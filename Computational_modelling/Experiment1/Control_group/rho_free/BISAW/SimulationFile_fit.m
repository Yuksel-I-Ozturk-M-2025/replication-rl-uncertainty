...
paramVector(2) = 1000; % Ns - nb of simulations to use to estimate the action likelihoods - Ns = 1000
paramVector(3) = 2.5; % r0 - reward of correct action - r0 = 10
paramVector(4) = 0.65; % rho - level of confidence (in perception)
...
Acc_BISAW = zeros(N,J); % For each subject's fitted parameters, one set of learning curves was obtained by averaging over nb_rep (100) simulations of the model 
...
R_rel_BISAW = zeros(N,J); % For each subject's fitted parameters, one set of learning curves was obtained by averaging over nb_rep (100) simulations of the model 
...
rho_vals = MatFittedParams(:,2);
b_vals = MatFittedParams(:,3);
w_vals = MatFittedParams(:,4);
pr_vals = MatFittedParams(:,5);
T_vals = MatFittedParams(:,6);
