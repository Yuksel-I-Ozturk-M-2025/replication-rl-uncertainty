...
% See also: read_stl
  ...
  for index=1:numel(OBJ.objects)
    if OBJ.objects(index).type == 'f'
      data.faces   = OBJ.objects(index).data.vertices;
      break
    end
  end

  data.vertices= OBJ.vertices;
  data.Title   = [ filename ' ' strtrim(OBJ.objects(1).data) ];
  data.Normals = OBJ.vertices_normal;
